...
// Bilingual learning objective schema
export const bilingualObjectiveSchema = z.object({
  en: z.string().describe("Learning objective in English"),
  zh: z.string().describe("Learning objective in Chinese"),
});
...
export const languagePointSchema = z.object({
  point: z
    .string()
    .describe('The language point title, e.g., "Present Perfect Tense"'),
  explanation: z.string().describe("Simple English explanation of the point"),
  explanationZH: z.string().describe("Chinese explanation of the point"),
...
export const paragraphAnalysisSchema = z.object({
  ...
  summaryZH: z.string().describe("One-sentence summary in Chinese"),
  languagePoints: z
    .array(languagePointSchema)
    .max(3)
    .describe("2-3 key language points"),
});
...
export const vocabularyItemSchema = z.object({
  ...
  originalSentence: z
    .string()
    .describe("The sentence from the article containing this word"),
  category: z
    .enum(["essential", "transferable", "extended"])
    .describe(
...
export const quizQuestionSchema = z.object({
  ...
  sourceReference: z
    .string()
    .describe("Quote from article that contains the answer"),
});

// Full course analysis schema
export const courseAnalysisSchema = z.object({
  learningObjectives: z
    .array(bilingualObjectiveSchema)
    .min(3)
    .max(5)
    .describe(
      "3-5 bilingual learning objectives, each with English and Chinese versions",
    ),
  paragraphs: z
  ...
  vocabulary: z
    ...
    .min(8)
    .max(15)
    .describe("8-15 key vocabulary items from the article"),
  quizQuestions: z
...
export const difficultySchema = z.object({
  ...
  advancedVocabPercentage: z
    .number()
    .describe("Percentage of vocabulary beyond the assessed level"),
...
export type BilingualObjective = z.infer<typeof bilingualObjectiveSchema>;
